...
class Client:
    ...
    base_url: str = "https://identity.spireon.com"
...
class AuthenticatedClient(Client):
    ...
    def from_login(cls, username: str, password: str):
        """Create a new client from a username and password"""
        return cls().with_login(username, password)

    def with_login(self, username: str, password: str):
        authtoken = base64.b64encode(bytes("%s:%s" % (username, password), "utf-8")).decode("ascii")
        return self.with_headers({"Authorization": f"Basic {authtoken}"})
